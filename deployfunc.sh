...
terraform init -backend-config="storage_account_name=$TERRAFORM_STORAGE_NAME" -backend-config="container_name=tfstate" -backend-config="access_key=$TERRAFORM_STORAGE_KEY" -backend-config="key=codelab.microsoft.tfstate" ./terraformfunc

echo "planning terraform..."
terraform plan -out $deployment_name-out.plan -var="deployment_name=$deployment_name" -var="location=$location" -var="tenant_id=$tenantid" -var="object_id=$objectid" -var="subscription_id=$subscriptionid" ./terraformfunc #-state=PATH=$deployment_name
