...
resource "azurerm_storage_account" "function3_storage" {
  name                     = "${var.deployment_name}func3"
...
resource "azurerm_app_service_plan" "function3_plan" {
  name                = "${var.deployment_name}func3plan"
  resource_group_name      = azurerm_resource_group.funcrg.name
  location                 = azurerm_resource_group.funcrg.location
...
resource "azurerm_function_app" "function3_app" {
  name                      = "${var.deployment_name}func3app"
  resource_group_name      = azurerm_resource_group.funcrg.name
  location                 = azurerm_resource_group.funcrg.location
  app_service_plan_id       = azurerm_app_service_plan.function3_plan.id
  storage_account_name       = azurerm_storage_account.function3_storage.name
  storage_account_access_key = azurerm_storage_account.function3_storage.primary_access_key
  ...
  site_config {
    ...
    #always_on         = true
    linux_fx_version  = "DOCKER|denniszielke/springrestmessaging"  #"DOCKER|mcr.microsoft.com/azure-functions/dotnet:2.0-appservice-quickstart"
